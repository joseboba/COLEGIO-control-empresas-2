...
function saveEmployee(req, res){
    ...
    }else if(params.name && params.lastname && params.email && params.email && params.dpi && params.departament && params.position && params.enterprise){
            ...
            if(err){
                ...
                employee.save((err, employeeSaved) =>{
                    if(err){
                        ...
                        Enterprise.findByIdAndUpdate(id, {$push:{employees: employee}}, {new: true}, (errE, enterpriseUpdated) =>{
                            if(errE){
                                res.send(errE);
                            }else if(enterpriseUpdated){
                                res.send({message: 'Se ha guardado el siguiente empleado:', employeeSaved});
                            }else{
                                Employee.findByIdAndRemove(employee._id, (err, employeeDelete) =>{
                                    if(err){
                                        res.send(err);
                                    }else if(employeeDelete){
                                        res.send({message: 'Como no se ha asignado a una empresa se ha eliminado el usuario'})
                                    }
                                });
                            }
                        });
            ...
            if(err){
                ...
                Employee.findByIdAndRemove(employeeId, (err, employeeDelete)=>{
                    if(err){
                        ...
                        Enterprise.findByIdAndUpdate(id, {$pull:{employees:{$in: [{_id:employeeId}]}}},{new: true}, (err, employeeDeleted)=>{
                            if(err){
                                res.status(500).send(err);
                            }else if(employeeDeleted){
                                res.send({message: 'El empleado eliminado fue:', employeeDeleted, employeeDelete});
                            }else{
