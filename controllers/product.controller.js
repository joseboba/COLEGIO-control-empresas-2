                ...
                if(params.name && params.stock && params.description){
                    ...
                    product.save((err, saved)=>{
                        if(err){
                            ...
                            Enterprise.findByIdAndUpdate(enterpriseId, {$push:{products:product}}, {new: true}, (err, savedP) =>{
                                if(err){
                                    res.status(500).send(err);
                                }else if(savedP){
                                    res.send({saved, savedP});
                                }else{
                                    res.status(418).send({message: 'No se ha podido agregar el producto al stock de la empresa'})
                                }
                            });
                        }else{
                            res.status(418).send({message: 'No se ha guardado el producto'});
                        }
                    });
                }else{
...
function removeProduct(req, res){
    ...
    if(enterpriseId != req.enterprise.sub){
        ...
        if(storeOrEnterprise.params == 'Store'){
                ...
                if(err){
                    ...
                    Store.findById(storeId, (err, find) =>{
                        if(err){
                            ...
                            Store.findByIdAndUpdate(storeId, {$pull:{product:{_id:storeOrEnterprise.id}}}, {new: true}, (err, deleted)=>{
                                if(err){
            ...
            Enterprise.findByIdAndUpdate(enterpriseId, {$pull:{products:id}}, {new: true}, (err, deletedE)=>{
                    if(err){
                        res.status(500).send(er);
                    }else if(deletedE){
                        Store.findById(storeId, (err, find) =>{
...
function updateProduct(req, res){
    ...
    if(enterpriseId != req.enterprise.sub){
        ...
        if(params.stock){
            Product.findByIdAndUpdate(id, {stock: stock + params.stock}, {new: true}, (err, updated) =>{
                if(err){
                    res.status(500).send(err);
                }else if(updated){
                    res.send({message: 'Se ha actualizado el stock correctamente:' , updated})
                }else{
