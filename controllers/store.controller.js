...
function saveStore(req, res){
    ...
    let store = new Store();
        ...
        }else if(params.direction && params.phone && params.manager && params.email){  
                ...
                if(err){
                    ...
                    Store.findOne({phone: params.phone}, (err, found) =>{
                        if(err){
                            ...
                            res.status(406).send({message: 'No puede crear una empresa con un telefono ya existente'});
                        }else{
                            ...
                            if(err){
                                ...
                                Enterprise.findByIdAndUpdate(id, {$push:{stores: store}}, {new: true}, (err, enterpriseUpdate) =>{
                                    if(err){
                                        res.status(500).send(err);
                                    }else if(enterpriseUpdate){
                                        res.send({message: 'Se ha guradado la siguiente tienda: ', storeCreate});
                                    }else{
                                        res.status(418).send({message: 'No se pudo crear la tienda'});
                                    }
                                });
                            }else{  
                                res.status(418).send({message: 'No se ha podido guardar la empresa'})
                            }
                    });
                        }
                    });
                }else{
                    res.status(404).send({message: 'La empresa no existe, no puede crear tiendas'});
                }
            });
        }else{
    ...
    if(enterpriseId != req.enterprise.sub){
        ...
        Store.findOne({phone: params.phone}, (err, found) =>{
            if(err){
                ...
                res.status(406).send({message: 'No puede cambiar el teleono a uno ya existente'});
            }else{
                Store.findByIdAndUpdate(id, params, {new: true}, (err, storeChanged) =>{
                    if(err){
                        res.status(500).send(err);
                    }else if(storeChanged){
                        res.send(storeChanged);
                    }else{
                        res.status(418).send({message: 'No se pudo actualizar la tienda'});
                    }
                });
            }
        });
    ...
    if(enterpriseId != req.enterprise.sub){
       ...
       Store.findByIdAndRemove(storeId, (err, deleted) =>{
            if(err){
                ...
                Enterprise.findByIdAndUpdate(enterpriseId, {$pull: {store: storeId}}, {new: true}, (err, updated)=>{
                    if(err){
                        res.status(500).send(err);
                    }else if(updated){
                        res.send({message: 'Se ha eliminado la seiguiente tienda: ', deleted});
                    }
                });
            }else{
