    ...
    if(params.user && params.password){
        Enterprise.findOne({user: params.user}, (err, enterpriseFound) =>{
            if(err){
                ...
                bcrypt.compare(params.password, enterpriseFound.password, (err, password)=>{
                    if(err) 
                        res.status(500).send(err)
                    else if(password)
        ...
        res.status(404).send({message: 'Ingrese los datos necesarios para iniciar sesion'});
    }
}
...
function saveEnterprise(req, res){
    ...
    if(params.CEO && params.name && params.email && params.phones && params.direction && params.socialApproach && params.password && params.CEO){
...
function updateEnterprise(req, res){
    ...
    if(enterpriseId != req.enterprise.sub){
        res.status(403).send({message: 'No tiene permisos para ingresar a la ruta'});
    }else{ 
                ...
                if(found.employees.length  > 0){
                    Employee.deleteMany({enterprise: found.name}, (err, deleted)=>{
                        if(err){
                                ...
                                Store.deleteMany({_id:found.stores}, (err, deleted2) =>{
                                    if(err){
                                        ...
                                        if(found.prodcts.length > 0){
                                            Product.deleteMany({_id:found.products}, (err, deleted3) =>{
                                                if(err){
                                                    res.status(500).send(err);
                                                }else if(deleted3){
                                                    res.send(found);
                                        ...
                                        res.status(404).send({message: 'No hay registros de tiendas'});
                                    }
                                });
                            }else{
                                res.send(found);
                            ...
                            res.status(404).send({message: 'No hay registros de empleados'});
                        }
                    });
                }else{
                    res.send(found);
...
function setProduct(req, res){
    ...
    if(enterpriseId != req.enterprise.sub){
        ...
        Store.findOne({_id:storeId, product:{$elemMatch:{products:params.product}}}, (err, found) =>{
            if(err){
                res.status(500).send(err);
            }else if(found){
                ...
                Product.findById(params.product, (err, pro) =>{
                    if(err){
                        ...
                        Product.findByIdAndUpdate(params.product, { stock: newQuantity, $push: {store: storeId } }, { new: true }, (err, added) => {
                            if (err) {
                                res.status(500).send({ error: 'Error interno del servidor', err });
                            } else if (added) {
                                Store.findByIdAndUpdate(storeId, { $push: { product: { products: params.product, quantity: params.quantity } } }, { new: true }, (err, updated) => {
                                    if (err) {
                                        res.status(500).send({err});
                                    } else if (updated) {
                                        res.send({ 'Producto asignado': updated.products, updated });
                                    } else {
    ...
    if(id != req.enterprise.sub){
        ...
        Product.find({}, (err, found)=>{
            if(err){
                ...
                res.send({message: 'Estos son los productos existentes, stock actual y a que sucursal pertenece:', found});
            }
        });
    }
